...
## ★ 硬性约束（最高优先级，实施时必须反复确认）

> **以下三条约束贯穿全部 Phase，违反任何一条即为实施失败。**

### 约束 1：保留 DuckDB 原生 CSV 加载支持
- Phase 0B 的 `_load_csv_native()` 是**必须实现**的功能，不得删除或跳过
- CSV 和 Excel 是两条并行的加载路径，互不替代
- 即使本次样本全部为 Excel 文件，CSV 原生加载仍必须完整实现并测试

### 约束 2：保持程序兼容性，不得固化到样例文件
- 提供的 14 个样例文件仅代表**部分**用户应用场景，不代表所有用户场景
- 所有优化（逐Sheet加载、日期提取、自动加载等）必须是**通用机制**，适用于任意 CSV/Excel 文件
- `auto_load.file_rules` 中的规则是**用户可配置的示例**，不是硬编码逻辑
- 代码中不得出现针对特定文件名的 if/else 判断
- `auto_detect_table_type()` 保持通用匹配逻辑，新增 table_type 通过配置而非代码添加

### 约束 3：必须在新版本分支上开发
- **绝对禁止**在当前 `claude/happy-euler-awvv3j` 分支上直接修改
- 从 `main` 分支创建新分支 `feature/large-data-adaptation`
- 所有改动在新分支上开发、测试、提交
...
- **代表性样本**：用户提供的 14 个 Excel 文件仅代表其部分典型场景，程序必须同时支持 CSV 和 Excel，且不能固化到这些样例文件
- **数据量**：每日 14 个文件，原始总计 ~490MB，~115 万行
- **使用模式**：文件名含日期，结构固定，内容每日更新
- **特殊处理**：标题行、多Sheet合并、千分位分隔符、特定Sheet选择
- **兼容性要求**：所有优化必须是通用机制，同时适用于 CSV 和 Excel 场景，不能固化到特定文件名或文件结构
