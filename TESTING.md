...
| **记录问题** | 发现任何不符合用户预期的地方都要记录，即使不影响功能运行 |
| **每次迭代必做** | 每次发布新版本前（含 beta），必须执行一轮 UAT |
...
| beta 版发布前 | 全量 UAT（所有 Skill + 所有面板） | 45-60 分钟 |
| 正式版发布前 | 全量 UAT + 跨版本回归 | 60-90 分钟 |
...
步骤 4: 设置与配置测试
  ...
  ├─ 修改配置 → 保存 → 重启验证持久化
  ├─ 测试 API Key 不泄露（GET /api/config 返回 api_key_set 而非实际 key）
  └─ 测试集团 CRUD、数据表卸载、会话删除

步骤 5: 回归冒烟
  ├─ 至少 2 轮自然语言对话，确保流式输出正常
  ├─ SSE 事件类型完整（thinking / tool_start / tool_end / text / done）
  ├─ 错误提示友好（不暴露技术堆栈）
  └─ 内存无明显增长（通过 /api/health 多次检查 RAM）

步骤 6: 问题记录与跟踪
  ├─ 每个问题标注优先级 P0/P1/P2/P3
  ├─ P0（阻断）：功能不可用或数据错误 → 立即修复
  ├─ P1（严重）：核心路径受阻 → 本次迭代修复
  ├─ P2（体验）：使用不便或提示不友好 → 下次迭代修复
  └─ P3（建议）：锦上添花 → 记录到测试债务
```
...
- [ ] 上传 CSV → 返回成功 + 质量报告
- [ ] 简单查询 → 回复准确、语言自然
- [ ] 聚合统计 → 触发 tool-calling（profile_table / run_sql）
- [ ] 固化计算 → 触发 run_calculator（非 LLM SQL）
- [ ] 流式输出 → 逐字推送，无长时间空白
- [ ] 错误处理 → 空消息/不存在表 → 友好提示

**v1.6 特性**
- [ ] Skill Builder：AI 生成 → 校验 → 保存草稿 → 发布
- [ ] Skill Builder：发布后 Skill 列表增加，Agent 可识别
- [ ] Runtime Logger：basic/detailed 模式切换即时生效
- [ ] Runtime Logger：日志读取、过滤、清理正常
- [ ] Settings：用户档案/阈值/记忆/调度 配置保存持久化
- [ ] Settings：API Key 不泄露
