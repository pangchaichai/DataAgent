...
UAT（User Acceptance Testing）是从**真实用户视角**出发的端到端验证，目的在于发现单元测试和集成测试无法覆盖的功能问题、数据问题和使用体验问题。

**核心原则：模拟真实用户，而非测试工程师。**

| 原则 | 说明 |
|------|------|
| **用户视角** | 以投资业务人员的身份操作，使用真实业务语言提问，而非技术术语 |
| **真实场景** | 使用实际业务数据（CSV 文件），构造日常工作中会遇到的查询和分析需求 |
| **全链路覆盖** | 从上传数据 → 自然语言交互 → 结果验证 → 新功能体验 → 设置管理的完整流程 |
| **不修复只记录** | UAT 过程中发现的问题只记录不修复，测试完成后再统一评估和安排修复 |
| **每次迭代必做** | 每次发布新版本前，必须执行一轮 UAT |

### 9.2 UAT 执行时机

| 时机 | 范围 | 时长 |
|------|------|------|
| 日常迭代后 | 变更模块 + 关联功能 | 15-30 分钟 |
| beta 版发布前 | 全量 UAT（所有 Skill + 所有 API + 所有面板） | 45-60 分钟 |
| 正式版发布前 | 全量 UAT + 跨版本回归 + 前端 JS 覆盖 | 60-90 分钟 |
...
步骤 1: 环境准备
  ├─ 启动应用（开发模式或生产模式）
  ├─ 准备真实业务数据文件（CSV/PDF）
  ├─ 确认 LLM 连接正常（/api/health）
  └─ 记录初始状态（表数量、Skill 数量、会话数）

步骤 2: 核心功能测试（从用户视角）
  ├─ 上传至少 2 种类型的 CSV（如持仓 + 评级）
  ├─ 自然语言提问（不涉及 SQL，以业务人员口吻）
  │   ├─ 简单查询：「帮我看看有哪些产品」
  │   ├─ 聚合统计：「每个产品的总市值是多少」
  │   ├─ 多表关联：「持仓表中的主体评级分布如何」
  │   └─ 模糊提问：「最近持仓情况怎么样」
  ├─ 验证回复内容：数据准确、语言自然、无技术堆栈
  └─ 验证 tool-calling 流程：是否正确使用了 profile_table / run_sql 等工具

步骤 3: API 端点全覆盖测试
  ├─ 逐一测试所有 API 端点（GET/POST/DELETE）
  ├─ 测试正常路径 + 边界条件（空输入/超长输入/不存在资源）
  ├─ 验证错误响应格式（ok:false + error message）
  └─ 检查 API Key 不泄露（/api/config 返回 api_key_set 而非实际 key）

步骤 4: 新功能测试
  ├─ 逐一测试本次迭代新增的功能模块
  ├─ 验证新功能与旧功能的交互是否正常
  ├─ 测试边界条件
  └─ 如 Skill Builder：AI 生成 → 校验 → 草稿 → 发布全链路

步骤 5: 设置与配置测试
  ├─ 测试设置面板各项开关（记忆/调度/日志模式）
  ├─ 修改配置 → 保存 → 重启验证持久化
  ├─ 测试集团 CRUD、数据表卸载、会话删除
  └─ 测试文件上传重复检测

步骤 6: 回归冒烟
  ├─ 至少 2 轮自然语言对话，确保流式输出正常
  ├─ SSE 事件类型完整
  ├─ 错误提示友好（不暴露技术堆栈）
  └─ 内存无明显增长

步骤 7: 问题汇总分析
  ├─ 每个问题标注优先级 P0/P1/P2/P3
  ├─ 整理成问题清单，供后续修复参考
  └─ 生成 UAT 测试报告
```
...
| 优先级 | 定义 | 示例 |
|--------|------|------|
| **P0** | 阻断：核心功能不可用或数据错误 | 查询返回空结果、Agent 崩溃、数据计算错误 |
| **P1** | 严重：重要路径受阻或安全风险 | API 返回格式错误、按钮无响应、会话丢失 |
| **P2** | 体验：使用不便或提示不友好 | 报错不友好、UI 状态混乱、缺少确认提示 |
| **P3** | 建议：锦上添花的改进 | 文案优化、性能微调、样式改进 |
...
- [ ] 上传 CSV → 返回成功 + 质量报告
- [ ] 简单查询 → 回复准确、语言自然
- [ ] 聚合统计 → 触发 tool-calling
- [ ] 固化计算 → 触发 run_calculator（非 LLM SQL）
- [ ] 流式输出 → 逐字推送，无长时间空白
- [ ] 错误处理 → 空消息/不存在表 → 友好提示

**API 端点**
- [ ] GET /api/health → 返回 LLM 状态和内存
- [ ] GET /api/tables → 已加载表列表
- [ ] POST /api/upload → 文件上传 + 质量报告
- [ ] POST /api/chat → 返回 stream_id
- [ ] GET /api/stream/<sid> → SSE 事件流
- [ ] GET /api/skills → Skill 注册表
- [ ] GET /api/config → 不泄露 API Key
- [ ] POST /api/config → 保存并持久化
- [ ] DELETE /api/tables/<name> → 卸载表
- [ ] DELETE /api/sessions/<id> → 删除会话

**v2.0 新特性**
- [ ] 联网搜索工具可用
- [ ] 文档解析工具（PDF）可用
- [ ] Plan-Execute Agent 规划正确
- [ ] 新 calculators（leverage/liquidity/position_diff）数据正确
- [ ] JS 模块化后 UI 功能完整
- [ ] Blueprint API 路由全部可达
...
- [ ] 上传重复文件时有提示
- [ ] 删除操作有确认或明确反馈
- [ ] 所有 API error 返回统一格式 {ok:false, error:msg}

### 9.6 UAT 结果记录

UAT 结束后在 `data/test_reports/` 生成记录：

**文件命名**：`uat_{YYYY-MM-DD}_{version}.md`
